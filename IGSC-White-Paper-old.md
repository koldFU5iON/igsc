# International Game Project Classification (IGPC)
...
The **International Game Project Classification (IGPC)** proposes a formal, bond-style rating system that rates **projects**, not studios. Drawing inspiration from financial credit ratings, IGPC uses the familiar AAA/AA/A/BBB/BB/B/C scale to indicate production capacity and resource backing. The framework evaluates contributing **sources** independently (studios, publishers, funders) and combines them into a composite project rating.
...
**For platform operators, awards bodies, grant programmes, and publishers evaluating IGPC adoption.**
...
**Example Project Rating:** `A/I1 — Verified — v0.5`
...
**1. Awards Bodies:** Use IGPC capacity tiers for fair competition categories
...
**2. Grant Programmes:** Use IGPC source ratings for eligibility criteria
...
**3. Platforms:** Integrate IGPC ratings into discovery and developer programmes
...
**For skim readers:** IGPC ratings describe production capacity and resource backing. They do NOT measure:
...
IGPC provides **context**, not **judgement**. A C-rated masterpiece is still a masterpiece. A AAA-rated failure is still a failure. The rating helps stakeholders understand the production scale and resource environment, not the outcome.
...
This terminology gap creates practical problems across the industry: unfair competition in awards, unclear eligibility for grants, inconsistent media coverage, and difficulty for studios to position themselves accurately. The International Game Project Classification (IGPC) addresses this gap by proposing a formal, multi-dimensional taxonomy built on verifiable structural criteria rather than subjective cultural labels.
...
The IGPC addresses these failures through a **source-based project rating system**.

### 4.1 Projects Are Rated, Not Studios

Unlike traditional approaches that classify studios as "indie" or "AAA," IGPC rates **individual projects**. This reflects the reality that studios commit different resources to different projects, and the same studio may simultaneously have projects operating at different scales with different goals.

**Studios do not have a single IGPC rating; projects do. Studios are evaluated as sources that contribute to project ratings, but they themselves are not classified or ranked in aggregate.**
...
IGPC adopts the bond rating nomenclature familiar from financial markets: **AAA, AA, A, BBB, BB, B, and C** (with modifiers + and - where applicable).
...
For game projects, IGPC borrows **only the scale**, not the underlying concept of financial risk. The ratings indicate **production capacity and resource backing**, not creditworthiness, commercial viability, or default probability.

**Rationale for simplified lower tiers:** Financial rating systems extend to CCC, CC, and C for granular distinctions in distressed debt scenarios. IGPC uses a single C tier for solo developers and micro-teams because further sub-segmentation provides little practical value in game industry contexts. The difference between a solo developer and a 2-person partnership is less relevant to awards categories, grant eligibility, or platform curation than the difference between C-tier and B-tier projects.

**Key distinctions:**

| Bond Ratings (S&P, Moody's, Fitch) | IGPC Capacity Ratings |
|-------------------------------------|----------------------|
...
The game industry already uses "AAA" and "AA" informally, but without clear definitions or a complete scale. IGPC formalises this familiar terminology into a structured framework, avoiding the adoption friction that entirely new terminology would face. The scale is recognisable and intuitive, even though the underlying measurement differs from financial credit ratings.

Throughout this document, we refer to IGPC outputs as **capacity ratings** to distinguish them clearly from financial credit ratings, while acknowledging the borrowed nomenclature.
...
IGPC provides two complementary outputs for each project:
...
Unlike systems that classify games after release based on output metrics, IGPC rates projects at any stage: before development begins, during production, or after release. This enables grant eligibility determination, award category placement, publisher scouting, and project positioning from day one.
...
IGPC distinguishes between publicly disclosed publisher relationships and undisclosed financial backing:
...
The IGPC rating process follows six steps:
...
Once sources are independently rated, they are combined to produce a project rating. IGPC uses a **weighted floor-and-ceiling model** that recognises both studio capacity constraints and external resource elevation.
...
This appendix provides the operational specification for the IGPC rating process. The form below can be completed by project leads in approximately 10 minutes and contains all information required to generate a project rating.
...
IGPC ratings can be verified at three distinct levels, each providing a different degree of confidence in the accuracy of self-reported information. Verification is optional but recommended for projects seeking maximum credibility.
...
- Evidence reviewed by IGPC registry (manual or automated spot-checking)
...
- Project provides confidential evidence to auditor (NOT to IGPC registry):
...
- Auditor issues attestation letter to IGPC registry confirming:
...
- IGPC registry records auditor attestation
...
- Projects pay audit fee directly to auditor (not to IGPC registry)
...
| **Who reviews** | No one | IGPC registry (spot-checking) | Independent third-party auditor |
...
The IGPC framework defines WHAT verification tiers are and WHY they matter. Operational details (how auditors are accredited, pricing structures, certification bodies) will be defined in a separate implementation guide as the framework matures.
...
- **Rated:** 2025-10-15
- **Version:** v0.5
- **Verification ID:** XK9P2-A7L3-M4R8
...
**Validation Approach:**
- **Expert Panel Review:** 15-20 game industry professionals (developers, publishers, journalists, researchers) independently classify the 100 test projects using the IGPC rubric
...
**Solution:** Create award categories by IGPC project rating tiers:
...
**Solution:** Filter and search projects by IGPC rating and source composition:
...
**Solution:** Define grant eligibility using IGPC source ratings:
...
**Solution:** Standardised IGPC project ratings enable:
...
**Solution:** IGPC project ratings provide:
...
**Solution:** Platforms can integrate IGPC ratings into:
...
This section examines existing approaches to studio and project classification within gaming and adjacent creative industries, providing context for the IGPC framework's design decisions.
...
**Limitations for Project Rating:** While HCS provides valuable market analysis, it addresses a different problem than IGPC:
...
**Complementary Approaches:** IGPC and HCS solve different problems and could coexist. IGPC rates the project's resource backing and source structure; HCS classifies the output's production scope based on credits and file size. A BBB-rated project by IGPC might produce a Midi-scale game by HCS metrics, indicating the team punched above their weight in execution.
...
The IGPC addresses these gaps by providing an open, platform-agnostic, multi-dimensional framework with transparent criteria.

---

## 10. Design Principles

The IGPC is built on these non-negotiable principles:
...
**Core principle:** IGPC measures **capacity** (infrastructure, headcount, track record), not **burn-rate equivalence** or purchasing power parity.
...
IGPC is a **production capacity framework**, not a financial equivalence model. The rating answers: "What resources and infrastructure does this project have access to?" not "What would this budget buy in San Francisco?"
...
2. **Avoids unverifiable claims:** Cost-of-living adjustments require detailed financial disclosure (exact salaries, regional multipliers) that conflicts with IGPC's principle of non-sensitive disclosure.
...
This approach will face criticism. A $1M budget in Warsaw objectively produces more output than a $1M budget in San Francisco due to salary cost differentials. IGPC explicitly **does not** attempt to normalize for this. The framework describes structural capacity and resource backing, not output efficiency per dollar.
...
- Publishers and investors should apply their own regional cost analysis when evaluating projects; IGPC provides capacity context, not ROI prediction
...
IGPC follows this model. The framework becomes valuable when projects self-rate, when awards bodies use IGPC ratings for categories, when grant programmes reference IGPC for eligibility, and when publishers and platforms filter by IGPC ratings. Each adoption makes the next one easier, creating network effects that compound over time.

### 11.2 Current Governance

The IGPC is maintained by its creator as an open standard. The framework documentation is publicly available, and feedback is welcomed.
...
- Proposals submitted via GitHub Issues on the IGPC repository (or equivalent public forum)
...
- When 3+ major platforms or awards bodies have adopted IGPC
...
The IGPC is available for adoption by:

### Platforms and Engine Providers
- Integrate IGPC project ratings into game pages and discovery systems
...
- Create fair competition categories by IGPC rating tier (C/B, BB/BBB, A, AA/AAA)
...
- Define clear, verifiable eligibility criteria using IGPC source ratings
...
### Press and Analysts
- Use consistent IGPC ratings terminology in coverage and analysis
...
- **Attribution**: You must give appropriate credit to IGPC and Devon Stanton
...
**Devon Stanton**
Creator, International Game Project Classification (IGPC)

---

*International Game Project Classification (IGPC): Bringing clarity to an industry that deserves better.*
